...
function GrainsContent() {
  ...
  useEffect(() => { api.get<GrainData>('/user/grains').then(setData).catch(() => {}) }, [])

  return (
    <div class="space-y-8">
      <div class="text-center">
        <h1 class="text-3xl font-bold text-foreground">Meus Grãos</h1>
        <p class="text-muted-foreground mt-2">Quanto mais você aprende, mais grãos recebe</p>
      </div>
      <div class="grid grid-cols-3 gap-4">
        {[
          { label: 'Saldo', value: data?.balance || 0, color: 'text-amber-500' },
          { label: 'Ganhos', value: data?.total_earned || 0, color: 'text-green-500' },
          { label: 'Gastos', value: data?.total_spent || 0, color: 'text-red-500' },
        ].map(item => (
