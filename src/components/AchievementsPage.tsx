...
function AchievementsContent() {
  ...
  return (
    <div class="space-y-6">
      ...
      {unlocked.length > 0 && (
        <>
          <h2 class="font-semibold text-[var(--color-text-primary)]">Desbloqueadas ({unlocked.length})</h2>
          <div class="grid sm:grid-cols-2 gap-3">
            {unlocked.map(a => (
              <div class={`reader-card p-4 ${rarityColors[a.rarity] || rarityColors.common}`}>
                <div class="flex items-center gap-3">
                  <span class="text-2xl">{a.icon || '🏆'}</span>
                  <div><h3 class="font-semibold text-[var(--color-text-primary)] text-sm">{a.name}</h3><p class="text-xs text-muted-foreground">{a.description}</p></div>
                </div>
              </div>
            ))}
          </div>
      ...
      {locked.length > 0 && (
        <>
          <h2 class="font-semibold text-[var(--color-text-primary)]">Bloqueadas ({locked.length})</h2>
          <div class="grid sm:grid-cols-2 gap-3">
            {locked.map(a => (
              <div class="reader-card p-4 opacity-60">
                <div class="flex items-center gap-3">
